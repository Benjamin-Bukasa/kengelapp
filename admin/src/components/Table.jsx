...
const Tables = () => {
  ...
  const handleDeleteSelected = () => {
    const newData = data.filter(user => !selectedRows.includes(user.id));
    setData(newData);
      ...
      <div className="flex flex-wrap gap-4 justify-between">
        <div className="w-1/4 flex gap-1">
          <input
            ...
            className="px-4 py-2 border rounded w-full flex-1 outline-none"
          />
          <button
            onClick={applyFilters}
            className="px-4 py-2 bg-blue-500 text-white rounded hover:bg-orange-400 transition-all"
          >
            <Search />
          </button>
        </div>
          ...
          <thead className="bg-gray-100 text-xs uppercase text-gray-600">
            <tr>
              <th className="px-4 py-2 border text-center">
                <input
                  type="checkbox"
                  checked={displayedData.length > 0 && displayedData.every(user => selectedRows.includes(user.id))}
                  onChange={(e) => {
                    if (e.target.checked) {
                      setSelectedRows(displayedData.map(user => user.id));
                    } else {
                      setSelectedRows([]);
                    }
                  }}
